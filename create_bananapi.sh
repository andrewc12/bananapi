...
debootstrap="debootstrap"
 ...
 while :; do
     case $1 in
         qemu)   # Call the first stage, then exit.
             qemu=1
...
if [ $qemu -eq 1 ]; then apt-get install qemu-user-static -y; fi
...
$debootstrap --verbose --include=${kernel},locales,sunxi-tools $arch stretch /mnt http://ftp.debian.org/debian
