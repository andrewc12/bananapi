...
name=debianbanana$(date -I).img

cd /tmp
dd if=/dev/zero bs=1M count=2000 > /tmp/${name}

losetup -f --show -P /tmp/${name}
...
qemu-debootstrap --verbose --arch=armhf jessie /mnt http://ftp.debian.org/debian
...
apt-get -t jessie-backports install linux-image-armmp-lpae u-boot u-boot-tools flash-kernel sunxi-tools 
...
echo "LeMaker Banana Pi" >> /etc/flash-kernel/machine
#Kernel arguments:
echo 'LINUX_KERNEL_CMDLINE="console=ttyS0,115200 hdmi.audio=EDID:0 disp.screen0_output_mode=EDID:1280x1024p60 root=/dev/mmcblk0p1 rootwait panic=10 ${extra}"' >> /etc/default/flash-kernel
#Kernel modules
