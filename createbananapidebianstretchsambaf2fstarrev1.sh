...
#Base configuration files
 ...
 echo "/dev/mmcblk0p2  /           f2fs    relatime,errors=remount-ro        0       0" > /mnt/etc/fstab
 echo "/dev/mmcblk0p1  /boot           ext4    relatime,errors=remount-ro        0       0" >> /mnt/etc/fstab
...
#Install a few other things:
   apt-get install console-setup keyboard-configuration openssh-server ntp samba nfs-kernel-server lxc bridge-utils debootstrap btrfs-tools f2fs-tools -y
...
#Kernel arguments:
echo 'LINUX_KERNEL_CMDLINE="console=ttyS0,115200 hdmi.audio=EDID:0 disp.screen0_output_mode=EDID:1280x1024p60 root=/dev/mmcblk0p2 rootfs=f2fs rootwait panic=10 ${extra}"' >> /mnt/etc/default/flash-kernel
 chroot /mnt
flash-kernel
   passwd root
   adduser andrew
...
#Cleanup
 ...
 sync
cd /mnt
tar -cvf /tmp/debianbananaf2fs20151227.tar ./
 
 
 
 cd /tmp
dd if=/dev/zero bs=1M count=7000 > /storage/tmp/debianbananaf2fs20151227.img

losetup -f --show -P /storage/tmp/debianbananaf2fs20151227.img
...
#Create partition(s). ie one big partition beginning with sector 2048, type 83 (Linux)
 fdisk ${card}
  mkfs.f2fs ${card}${partition}
 mkfs.ext4 ${card}p1

 mount ${card}${partition} /mnt
 mkdir /mnt/boot
  mount ${card}p1 /mnt/boot
cd /mnt
tar -xvf /tmp/debianbananaf2fs20151227.tar ./
cd /storage/tmp

 umount /mnt/boot /mnt
 losetup -d /dev/loop0
 sync
gzip debianbananaf2fs20151227.img
